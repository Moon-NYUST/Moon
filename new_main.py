...
# 1. Google 登入換取自家 JWT 的接口 通過class A完成
@app.post("/auth/google/code", summary="Google OAuth 登入驗證")
async def google_auth_with_code(request: CodeRequest):
    """
    接收前端拿到的 Google code，透過交換 token 後驗證身分，再發放本系統的 JWT
    """
    # Step 1: 呼叫 google_oauth.py 交換 token
    tokens = exchange_code_for_tokens(request.code, request.redirect_uri)
    
    # Step 2: 呼叫 google_oauth.py 驗證身分
    google_id_token = tokens.get("id_token")
    if not google_id_token:
        raise HTTPException(status_code=400, detail="Google 未回傳 ID Token")
    
    user_info = verify_google_id_token(google_id_token)

    # Step 3: 取得使用者 email (通常作為 User Unique ID)
    user_email = user_info.get("email")
    if not user_email:
        raise HTTPException(status_code=400, detail="Google 帳號未提供 Email")
    
    # Step 4: 呼叫 auth_utils.py 簽發自家的 Access Token
...
# 1b. Google 登入換取自家 JWT 的接口 通過class B完成
@app.post("/auth/google", summary="[架構B] 用 ID Token 換取 JWT")
async def google_auth_with_id_token(request: TokenRequest):
    """
    接收前端拿到的 Google ID Token，透過驗證身分，再發放本系統的 JWT
    """
    # Step 1: 呼叫 google_oauth.py 驗證身分
    user_info = verify_google_id_token(request.id_token)

    # Step 2: 取得使用者 email (通常作為 User Unique ID)
    user_email = user_info.get("email")
    if not user_email:
        raise HTTPException(status_code=400, detail="Google 帳號未提供 Email")
    
    # Step 3: 呼叫 auth_utils.py 簽發自家的 Access Token
    access_token = create_access_token(data={"sub": user_email})
